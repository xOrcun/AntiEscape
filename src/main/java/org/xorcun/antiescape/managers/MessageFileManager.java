...
public class MessageFileManager {
    ...
    private String getMainCommand() {
        if (langConfig == null) return "control";
        List<String> aliases = langConfig.getStringList("main-command-aliases");
        if (aliases == null || aliases.isEmpty()) return "control";
        return aliases.get(0);
    }

    private String replacePlaceholders(String message) {
        if (message == null) return null;
        return message.replace("%main_command%", getMainCommand());
    }
    ...
    public List<String> getLangMessageListNoPrefix(String key) {
        ...
        messages.replaceAll(textToTranslate -> ChatColor.translateAlternateColorCodes('&', replacePlaceholders(textToTranslate)));
        return messages;
    }
